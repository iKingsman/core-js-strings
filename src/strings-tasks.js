...
function removeFirstOccurrences(str, value) {
  const indexOfStart = str.indexOf(value);
  return indexOfStart !== -1
    ? str.slice(0, indexOfStart) + str.slice(indexOfStart + value.length)
    : str;
}
...
function removeLastOccurrences(str, value) {
  const indexOfStart = str.lastIndexOf(value);
  return indexOfStart !== -1
    ? str.slice(0, indexOfStart) + str.slice(indexOfStart + value.length)
    : str;
}
...
function sumOfCodes(str) {
  let sum = 0;
  const strLength = str !== undefined && str !== null ? str.length : 0;
...
function countVowels(str) {
  ...
  if (str.length > 0) {
    for (let i = 0; i < str.length; i += 1) {
      if (volves.includes(str[i])) countVolves += 1;
    }
  }
  return countVolves;
}
...
function findLongestWord(sentence) {
  ...
  arr.map((item) => {
    if (item.length > largestWord.length) largestWord = item;
    return item;
  });
  return largestWord;
}
...
function encodeToRot13(str) {
  ...
  str.split('').map((item, index) => {
    if (letters.indexOf(item) !== -1) {
      const shift =
        (letters.indexOf(str[index]) + 13 + lettersCount) % lettersCount;
      res.push(
        str[index] === str[index].toUpperCase()
          ? letters[shift].toUpperCase()
          : letters[shift].toLocaleLowerCase()
      );
    } else res.push(item);
    return item;
  });
